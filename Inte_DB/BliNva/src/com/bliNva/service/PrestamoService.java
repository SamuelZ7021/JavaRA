...
public class PrestamoService implements PrestamoServiceInterface {
    ...
    private final PrestamoDAOImpl prestamoDAO;
    private final LibroDAOImpl libroDAO;
    private final SocioDAOImpl socioDAO;
    ...
    public Prestamo registrarPrestamo(int idLibro, int idSocio, int idUsuario) throws BusinessException {
        Optional<Libro> libroOpt = libroDAO.searchById(idLibro);
        if (libroOpt.isEmpty()) {
            throw new BusinessException("El libro con ID " + idLibro + " no existe.");
        }
        Libro libro = libroOpt.get();
        if (libro.getStockDisponible() <= 0) {
            throw new BusinessException("No hay stock disponible para el libro '" + libro.getTitulo() + "'.");
        }

        Optional<Socio> socioOpt = socioDAO.searchById(idSocio);
        if (socioOpt.isEmpty()) {
            throw new BusinessException("El socio con ID " + idSocio + " no existe.");
        }
        Socio socio = socioOpt.get();
        if (!socio.getEstado().equalsIgnoreCase("activo")) {
            throw new BusinessException("El socio '" + socio.getNombre() + "' no está activo.");
        }
        ...
        try {
            ...
            libroDAO.updateWithConnection(libro, conn);
    ...
    public Prestamo registrarDevolucion(int idPrestamo) throws BusinessException {
        ...
        Optional<Libro> libroOpt = libroDAO.searchById(prestamo.getIdLibro());
        if(libroOpt.isEmpty()){
